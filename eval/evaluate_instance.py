...
def run(args):
    ...
    encoder_ckpt = args.encoder_ckpt if args.encoder_ckpt is not None else cfg.get("encoder_ckpt")

    if encoder_ckpt and not os.path.exists(encoder_ckpt):
        print(f"[warn] encoder_ckpt not found: {encoder_ckpt}. Falling back to HF pretrained.")
        encoder_ckpt = None

    if args.use_hf_pretrained:
        use_hf_pretrained = True
    elif args.no_hf_pretrained:
        use_hf_pretrained = False
    else:
        use_hf_pretrained = bool(cfg.get("use_hf_pretrained", False))
    ...
    encoder = create_uni_encoder(
        ...
        use_hf_pretrained=use_hf_pretrained,
    )
    model = UNISegmentor(
        encoder=encoder,
...
def parse_args():
    ...
    parser.add_argument("--encoder_ckpt", type=str, default=None, help="Override encoder checkpoint path.")
    parser.add_argument("--use_hf_pretrained", action="store_true", help="Force HF pretrained weights.")
    parser.add_argument("--no_hf_pretrained", action="store_true", help="Disable HF pretrained weights.")

    # postprocess parameters
    # Default = Scheme A (balanced): improves AP without hurting AJI/F1 much.
    # Scheme B (more aggressive splitting): t_fg=0.55, d_sigma=1.0, min_distance=12,
    # peak_threshold=0.32, area_min=25 -> slightly higher AP, but lower AJI/F1.
    parser.add_argument("--t_fg", type=float, default=0.55, help="Foreground threshold.")
    parser.add_argument("--d_sigma", type=float, default=1.2, help="Gaussian sigma on distance map.")
    parser.add_argument("--min_distance", type=int, default=13, help="Minimum peak distance.")
    parser.add_argument("--peak_threshold", type=float, default=0.3, help="Minimum peak value.")
    parser.add_argument("--area_min", type=int, default=30, help="Min predicted instance size.")
